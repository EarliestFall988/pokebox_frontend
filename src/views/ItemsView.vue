<template>
    <main class="flex">
        ...
        <div class="w-full h-full m-2 rounded-lg p-2 text-white">
            ...
            <div v-if="pokemonOwned.length >= 1" class="flex justify-center items-center">
                <TransitionGroup name="list"
                    ...
                    <IconLoader v-for="p in pokemonOwned"
                                :key="p"
                                class="m-2"
                                :url="p.ItemTypeName"
                                :name="p.ItemImageLink"
                                :item="true"
        ...
        <div v-if="selectedItemName != null" class="hidden md:flex">
            <!-- <DetailsPanel /> -->
            <PanelView class="bg-gray-800/50 rounded-none m-0 w-64 flex justify-start"
                       title="Items"
                ...
                <Transition name="list" appear mode="out-in">
                    ...
                    <div class=" ">
                        <div class="w-full">
                            <img :src="selectedItemURL" class="w-full h-full" />
                        </div>
                        <p class="text-center w-[100%] font-bold text-lg truncate">
                            {{ selectedItemName.replace('-', ' ') }}
                        </p>
                        <p class="text-center w-[100%] italic truncate">
                            {{ selectedItemType.replace('-', ' ') }}
                        </p>
                        <p class="mt-5 w-full text-center">Description:</p>
                        <p class=" text-center w-full">
                            {{ selectedItemDescription.replace('-', ' ') }}
                        </p>
                    </div>
                </Transition>
            </PanelView>
...
<script setup>
    ...
    const selectedItemDescription = ref(null)

    const selectItem = function (name) {
        var res = pokemonOwned.value.find((p) => p.ItemName === name)

        selectedItemName.value = res.ItemImageLink
        selectedItemURL.value = res.ItemTypeName
        selectedItemType.value = res.ItemName
        selectedItemDescription.value = res.Description
    }
